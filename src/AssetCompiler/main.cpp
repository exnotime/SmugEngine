...
namespace smug {
	...
	int main(const uint32_t argc, const char** argv) {
		smug::Options opts = smug::ParseOptions(argc, argv);

		if (opts.Compile) {
			std::vector<std::string> files;

			std::string modelFolder = opts.AssetFolder + "/models";
			//smug::ScanDirForAssets(modelFolder, { ".obj", ".dae" }, files);

			std::string textureFolder = opts.AssetFolder + "/textures";
			//smug::ScanDirForAssets(textureFolder, { ".dds" }, files);

			std::string shaderFolder = opts.AssetFolder + "/shaders";
			smug::ScanDirForAssets(shaderFolder, { ".shader" }, files);

			smug::ResourceAllocator allocs;
			allocs.AllocResource = smug::AllocatorResource;
			allocs.UserData = nullptr;
			auto& assetLoader = smug::g_AssetLoader;
			...
			if (f) {
				...
				smug::LedgerEntry entry;
				while (fread(&entry, sizeof(smug::LedgerEntry), 1, f) == 1) {
					if (feof(f))
						break;
